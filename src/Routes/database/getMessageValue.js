...
module.exports = {
    path: "/api/database/get/message/:message",
    method: "get",
    /**
     * 
     ...
     * @example
     * $httpGet[REPL URI/api/database/get/message/$messageID?name=test]
     * 
     * $if[$httpResult[error]]
     *     An error occured.
     *     $stop
    ...
    async run (req, res, apisBuilder) {
        ...
        let value = await apisBuilder.db.get(name+"_"+message);
        return res.status(200).json({
            error: false,
            status: 200,
            data: {
                message: "Correctly fetched the message variable " + name + ".",
                value: value
            }
        });
    }
