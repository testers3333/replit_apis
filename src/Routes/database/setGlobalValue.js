...
module.exports = {
     ...
     * @example
     * $httpPost[REPL URI/api/database/set/global;{
     *   "name": "test",
     *   "value": "value of test"
     * }]
     * 
    ...
    async run (req, res, apisBuilder) {
        ...
        let old = await apisBuilder.db.get(name);
        apisBuilder.db.set(name, value);
        return res.status(200).json({
            error: false,
            status: 200,
            data: {
                message: "Correctly updated the global variable " + name + " value.",
                values: {
                    old: old,
                    new: value
                }
