...
module.exports = {
    path: "/api/database/set/user/:user",
    method: "post",
    /**
     * 
     ...
     * @example
     * $httpPost[REPL URI/api/database/set/user/$authorID;{
     *   "name": "test",
     *   "value": "value of test"
     * }]
     * 
    ...
    async run (req, res, apisBuilder) {
        ...
        let old = await apisBuilder.db.get(name+"_"+user);
        apisBuilder.db.set(name+"_"+user, value);
        return res.status(200).json({
            error: false,
            status: 200,
            data: {
                message: "Correctly updated the user variable " + name + " value.",
                values: {
                    old: old,
                    new: value
                }
