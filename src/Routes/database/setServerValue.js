...
module.exports = {
    path: "/api/database/set/guild/:guild",
    method: "post",
    /**
     * 
     ...
     * @example
     * $httpPost[REPL URI/api/database/set/guild/$guildID;{
     *   "name": "test",
     *   "value": "value of test"
     * }]
     * 
    ...
    async run (req, res, apisBuilder) {
        ...
        let old = await apisBuilder.db.get(name+"_"+guild);
        apisBuilder.db.set(name+"_"+guild, value);
        return res.status(200).json({
            error: false,
            status: 200,
            data: {
                message: "Correctly updated the guild variable " + name + " value.",
                values: {
                    old: old,
                    new: value
                }
