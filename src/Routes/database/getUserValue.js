...
module.exports = {
    path: "/api/database/get/user/:user",
    method: "get",
    /**
     * 
     ...
     * @example
     * $httpGet[REPL URI/api/database/get/user/$authorID?name=test]
     * 
     * $if[$httpResult[error]]
     *     An error occured.
     *     $stop
    ...
    async run (req, res, apisBuilder) {
        ...
        let value = await apisBuilder.db.get(name+"_"+user);
        return res.status(200).json({
            error: false,
            status: 200,
            data: {
                message: "Correctly fetched the user variable " + name + ".",
                value: value
            }
        });
    }
